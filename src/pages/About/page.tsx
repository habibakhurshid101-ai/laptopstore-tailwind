    ...
    <div className="min-h-screen bg-white dark:bg-[#080e1c] text-slate-900 dark:text-slate-50">
      ...
      <section className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
        <div className="flex-1 space-y-8">
          <span className="inline-block px-4 py-1.5 bg-[#1a56db]/10 border border-[#1a56db]/30 text-sky-400 text-sm font-semibold rounded-full uppercase tracking-wider">
            🏢 Our Story
          </span>
          <h1 className="text-5xl md:text-7xl font-extrabold text-slate-900 dark:text-slate-50 leading-tight tracking-tight">
            Pakistan's Most <span className="bg-gradient-to-r from-[#1a56db] to-[#0ea5e9] bg-clip-text text-transparent">Trusted</span> Laptop Store
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              className="px-10 py-5 bg-[#1a56db] text-white font-bold rounded-xl hover:bg-[#1a56db]/90 hover:scale-105 transition-all duration-300"
              onClick={() => navigate('/products')}
            >
              Shop Now
            </button>
            <button 
              className="px-10 py-5 bg-transparent border-2 border-[#1a56db] text-[#1a56db] font-bold rounded-xl hover:bg-[#1a56db] hover:text-white transition-all duration-300"
        ...
        <div className="flex-1">
          <div className="relative group">
            <div className="absolute -inset-4 bg-[#1a56db]/20 blur-[100px] rounded-full"></div>
            <img 
              src="/img1.avif" 
              alt="LapElite Store" 
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 py-16 border-y border-white/5 text-center mb-24">
          {[
            ...
            <div key={i} className="space-y-2">
              <h2 className="text-4xl font-extrabold text-[#1a56db]">{stat.val}</h2>
              <p className="text-sm text-slate-500 dark:text-slate-500 font-bold uppercase tracking-widest">{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-lg">The principles that guide everything we do</p>
        </div>
        <div className="flex flex-wrap justify-center gap-5 mb-24">
          {[
            ...
            <div key={i} className="w-[260px] p-8 flex flex-col items-center text-center bg-gray-50 dark:bg-[#0d1526] border border-black/10 dark:border-white/10 rounded-md space-y-4 hover:border-[#1a56db] transition-all duration-300">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-lg">The people behind LapElite</p>
        </div>
        <div className="flex flex-wrap justify-center gap-5 mb-24">
          {[
            ...
            <div key={i} className="w-[220px] p-8 bg-gray-50 dark:bg-[#0d1526] border border-black/10 dark:border-white/10 rounded-md text-center space-y-4 hover:border-[#1a56db] transition-all duration-300 group">
              <div className={`w-20 h-20 mx-auto flex items-center justify-center rounded-full text-4xl border-2 transition-all duration-500
                ${member.color === 'blue' ? 'bg-[radial-gradient(circle,rgba(26,86,219,0.2),transparent)] border-[#1a56db]' : ''}
                ${member.color === 'purple' ? 'bg-[radial-gradient(circle,rgba(124,58,237,0.2),transparent)] border-[#7c3aed]' : ''}
                ${member.color === 'sky' ? 'bg-[radial-gradient(circle,rgba(14,165,233,0.2),transparent)] border-[#0ea5e9]' : ''}
                ${member.color === 'green' ? 'bg-[radial-gradient(circle,rgba(6,214,160,0.2),transparent)] border-[#06d6a0]' : ''}
              `}>
                {member.icon}
              </div>
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-50 group-hover:text-[#1a56db] transition-colors">{member.name}</h3>
                <p className={`text-[10px] font-bold uppercase tracking-widest
                  ${member.color === 'blue' ? 'text-[#1a56db]' : ''}
                  ${member.color === 'purple' ? 'text-[#7c3aed]' : ''}
                  ${member.color === 'sky' ? 'text-[#0ea5e9]' : ''}
                  ${member.color === 'green' ? 'text-[#06d6a0]' : ''}
                `}>{member.role}</p>
                <p className="text-xs text-slate-500 dark:text-slate-500 leading-relaxed pt-2 line-clamp-3">{member.bio}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <section className="py-24 mb-24">
          <div className="p-12 md:p-20 bg-[#1a56db] rounded-[40px] flex flex-col lg:flex-row justify-between items-center gap-12 relative overflow-hidden text-center lg:text-left">
